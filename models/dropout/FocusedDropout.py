...
class FocusedDropout(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training: # training
            ...
            # 4. Generate mask
            rand = torch.rand(x.shape[0], x.shape[-2], x.shape[-1]).to(x.device)
            rand = rand * (self.high - self.low) + self.low # [0, 1] -> [low, high]
            back_max_channel = max_channel.reshape(x.shape[0], x.shape[-2], x.shape[-1])
            mask = (back_max_channel > rand * max_pixel).float() # 1 when max_channel is larger than rand number, similar to dropout # [m, h, w]
